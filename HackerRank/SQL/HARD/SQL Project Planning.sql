 ...
 *	                                                                            *
 *	If the End_Date of the tasks are consecutive, then they are part of         *
 *	the same project.                                                           *
 *	Samantha is interested in finding the total number                          *
...
SELECT MIN(START_DATE) AS START_DT, MAX(END_DATE) AS END_DT
FROM (	SELECT 
			  START_DATE
			, END_DATE
			, END_DATE - ROW_NUMBER() OVER(ORDER BY END_DATE ASC) TABI_GROUP
		FROM PROJECTS
	) PROJ
GROUP BY TABI_GROUP
ORDER BY COUNT(*), START_DT;